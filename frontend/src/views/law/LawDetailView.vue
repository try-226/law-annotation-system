...
import type { LawBaseInfo, LawDetail, LawStructureInput, StructureNodeType } from '../../types/law'
...
async function removeLaw() {
  if (!window.confirm('确认删除这部法律？当前实现按软删除处理，名称仍会保留。')) return
...
<template>
  <section class="law-page">
    <div class="page-title">
      ...
      <div class="actions"><RouterLink class="button secondary" to="/laws">返回列表</RouterLink><button class="danger" :disabled="!!saving" @click="removeLaw">删除法律</button></div>
    </div>
    ...
    <template v-else-if="detail">
      ...
      <div class="card">
        <h2>基础信息</h2>
        <div class="form-grid">
          <label class="field"><span>法律名称</span><input v-model="base.name" maxlength="100" /></label>
          <label class="field"><span>发布机关</span><input v-model="base.issuingAuthority" maxlength="100" /></label>
          <label class="field"><span>发布日期</span><input v-model="base.publicationDate" type="date" /></label>
          <label class="field"><span>效力状态</span><select v-model="base.validityStatus"><option value="ACTIVE">现行有效</option><option value="NOT_EFFECTIVE">尚未生效</option><option value="INVALID">失效</option><option value="REPEALED">已废止</option></select></label>
        </div>
        <div class="section-actions"><button :disabled="!!saving" @click="run('base', () => updateLawBase(lawId, base))">{{ saving === 'base' ? '保存中…' : '保存基础信息' }}</button></div>
      </div>

      <div class="card">
        <div class="row-head"><div><h2>结构</h2><p class="muted">结构调整仅写审计，不生成内容版本。</p></div><button class="secondary small" @click="addStructureNode">新增结构</button></div>
        <p v-if="structures.length === 0" class="muted">当前没有编、章、节结构。</p>
        <div v-for="node in structures" :key="node.nodeId" class="row-card">
          <div class="row-head"><strong>{{ node.title || '未命名结构' }}</strong><button class="danger small" @click="removeStructureNode(node.nodeId)">移除</button></div>
          <div class="row-grid">
            ...
            <label class="field"><span>上级结构</span><select v-model="node.parentNodeId"><option :value="null">无</option><option v-for="parent in structures.filter((item) => item.nodeId !== node.nodeId)" :key="parent.nodeId" :value="parent.nodeId">{{ parent.title || parent.nodeId }}</option></select></label>
            <label class="field full"><span>包含法条（可多选）</span><select v-model="node.articleRefs" multiple size="4"><option v-for="article in detail.articles" :key="article.articleId" :value="article.articleId">{{ article.number }}</option></select></label>
          </div>
        </div>
        <div class="section-actions"><button :disabled="!!saving" @click="run('structure', () => updateLawStructure(lawId, structures))">{{ saving === 'structure' ? '保存中…' : '保存结构' }}</button></div>
      </div>
      ...
      <div class="card">
        ...
        <div v-for="article in detail.articles" :key="article.articleId" class="row-card">
          <div class="row-head"><strong>{{ article.number }}</strong><div class="actions"><button class="small" :disabled="!!saving" @click="run(`article-${article.articleId}`, () => updateLawArticle(lawId, article.articleId, article))">保存</button><button class="danger small" :disabled="!!saving" @click="removeArticle(article.articleId)">删除</button></div></div>
          <div class="row-grid">
        ...
        <div class="row-card">
          ...
          <div class="row-grid">
            <label class="field"><span>条号</span><input v-model="newArticle.number" maxlength="50" /></label>
            <label class="field"><span>顺序</span><input v-model.number="newArticle.order" min="0" type="number" /></label>
            <label class="field full"><span>正文</span><textarea v-model="newArticle.body" rows="4"></textarea></label>
          </div>
          <div class="section-actions"><button :disabled="!!saving || !newArticle.number.trim() || !newArticle.body.trim()" @click="createArticle">{{ saving === 'new-article' ? '添加中…' : '添加法条' }}</button></div>
        </div>
      </div>
    </template>
  </section>
