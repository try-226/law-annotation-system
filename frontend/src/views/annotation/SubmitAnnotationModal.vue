...
<template>
  <AppModal :open="open" :title="title" :busy="busy" width="650px" @close="emit('close')">
    <div class="annotation-submit-copy">
      <p v-if="!locators.length">{{ description }}</p>
      <div v-else class="annotation-submit-warning"><strong>提交校验未通过：还有 {{ locators.length }} 项需要处理。</strong><span>点击错误项定位并补充保存。</span></div>
